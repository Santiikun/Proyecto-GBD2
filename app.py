...
#LISTO
@app.route('/listar_entidades_con_atributos')
def listar_entidades_con_atributos():
    try:
        conn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=.\SANTI;DATABASE=G_Hospital;UID=sa;PWD=123456')
        cursor = conn.cursor()
        cursor.execute(
        ...
        conn.close()
        return render_template('listar_entidades_con_atributos.html', entidades=entidades)
...
#LISTO
@app.route('/crear_tabla_auditoria')
def crear_tabla_auditoria():
    try:
            cursor = conn.cursor()
            cursor.execute("""
            IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'AUDITORIA') AND type in (N'U'))
            BEGIN
                CREATE TABLE AUDITORIA (
                    id INT PRIMARY KEY IDENTITY(1,1),
                    tabla_afectada NVARCHAR(50) NOT NULL,
                    fecha_hora DATETIME NOT NULL DEFAULT GETDATE(),
                    usuario_actual NVARCHAR(50) NOT NULL,
                    detalle_accion NVARCHAR(MAX) NOT NULL
                );
                PRINT 'Tabla de auditoría creada con éxito.';
            END
            """)
            conn.commit()
            return "Tabla de auditoría creada con éxito."
    except Exception as error:
        print(f"Error al crear la tabla de auditoría: {error}")
        return f"Error al crear la tabla de auditoría: {error}"
    
    
#TRIGGERS
@app.route('/crear_disparadores_auditoria')
def crear_disparadores_auditoria():
    try:
            cursor = conn.cursor()
            cursor.execute("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_CATALOG='G_Hospital';")
            tables = cursor.fetchall()
            ...
            CREATE TABLE AUDITORIA (
                ...
                usuario_actual NVARCHAR(255),
                detalle_accion NVARCHAR(255),
            ...
            for table in tables:
                ...
                insert_trigger = f"""
                CREATE OR ALTER TRIGGER trg_audit_insert_{table_name}
                ON {table_name}
                AFTER INSERT
                AS
                BEGIN
                    INSERT INTO AUDITORIA (tabla_afectada, usuario_actual, detalle_accion)
                    VALUES ('{table_name}', SYSTEM_USER, 'Se ha insertado un registro en {table_name}');
                    PRINT 'Trigger de insert activado';
                END
                """

                update_trigger = f"""
                CREATE OR ALTER TRIGGER trg_audit_update_{table_name}
                ON {table_name}
                AFTER UPDATE
                AS
                BEGIN
                    INSERT INTO AUDITORIA (tabla_afectada, usuario_actual, detalle_accion)
                    VALUES ('{table_name}', SYSTEM_USER, 'Se ha actualizado un registro en {table_name}');
                    PRINT 'Trigger de update activado';
                END
                """

                delete_trigger = f"""
                CREATE OR ALTER TRIGGER trg_audit_delete_{table_name}
                ON {table_name}
                AFTER DELETE
                AS
                BEGIN
                    INSERT INTO AUDITORIA (tabla_afectada, usuario_actual, detalle_accion)
                    VALUES ('{table_name}', SYSTEM_USER, 'Se ha eliminado un registro en {table_name}');
                    PRINT 'Trigger de delete activado';
                END
                """

                cursor.execute(insert_trigger)
                print(f"Trigger de auditoría de INSERT creado para la tabla {table_name} con éxito.")
                
                cursor.execute(update_trigger)
                print(f"Trigger de auditoría de UPDATE creado para la tabla {table_name} con éxito.")
                
                cursor.execute(delete_trigger)
                print(f"Trigger de auditoría de DELETE creado para la tabla {table_name} con éxito.")

            conn.commit()
            print("Triggers de auditoría creados para todas las tablas.")
            return "Triggers de auditoría creados con éxito."
    except Exception as error:
        print(f"Error al crear los triggers de auditoría: {error}")
        return f"Error al crear los triggers de auditoría: {error}"
...
@app.route('/crear_funcion_asignar_cama', methods=['GET', 'POST'])
def crear_funcion_asignar_cama():
    if request.method == 'POST':
        try:
            with get_db_connection() as conn:
                ...
                cursor.execute("""
                CREATE OR ALTER PROCEDURE asignar_cama (
                    @p_id_cama INT,
                    @p_id_paciente INT,
                    @p_id_medico INT,
                    @p_numero_cama CHAR(10),
                    @p_piso CHAR(10)
                )
                AS
                BEGIN
                    DECLARE @cama_existente INT;

                    -- Verificar si el paciente ya está asignado a otra cama
                    SELECT @cama_existente = ID_CAMA 
                    FROM CAMA 
                    WHERE ID_PACIENTE = @p_id_paciente;

                    IF @cama_existente IS NOT NULL
                    BEGIN
                        RAISERROR ('El paciente %d ya está asignado a otra cama', 16, 1, @p_id_paciente);
                        RETURN;
                    END

                    -- Verificar si el medico ya está asignado a otra cama
                    SELECT @cama_existente = ID_CAMA 
                    FROM CAMA
                    ...
                    IF @cama_existente IS NOT NULL
                    BEGIN
                        RAISERROR ('El medico %d ya está asignado a otra cama', 16, 1, @p_id_medico);
                        RETURN;
                    END

                    -- Asignar la cama
                    INSERT INTO CAMA (ID_CAMA, ID_PACIENTE, ID_MEDICO, NUMERO_CAMA, PISO)
                    VALUES (@p_id_cama, @p_id_paciente, @p_id_medico, @p_numero_cama, @p_piso);

                    PRINT 'Cama asignada correctamente: ' + CAST(@p_id_cama AS VARCHAR(10));
                END;
                """)

                conn.commit()
                return "Procedimiento 'asignar_cama' creado con éxito."
        
        except Exception as error:
            return f"Error al crear el procedimiento 'asignar_cama': {error}"
    
    return render_template('/crear_funcion_asignar_cama.html')




@app.route('/asignar_cama', methods=['GET', 'POST'])
def asignar_cama():
    if request.method == 'POST':
        id_cama = request.form['id_cama']
        id_paciente = request.form['id_paciente']
        id_medico= request.form['id_medico']
        numero_cama = request.form['numero_cama']
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                EXEC asignar_cama @p_id_cama=?, @p_id_paciente=?, @p_id_medico=?, @p_numero_cama=?, @p_piso=?;
            """, (id_cama, id_paciente, id_medico, numero_cama, piso))
            conn.commit()
            flash("Cama asignada correctamente.", "success")
        except Exception as error:
            flash(f"Error al asignar la cama: {error}", "danger")
        finally:
            if cursor:
                cursor.close()
            if conn:
                conn.close()
        return redirect(url_for('asignar_cama'))
    return render_template('asignar_cama.html')




@app.route('/imprimir_tablas')
def imprimir_tablas():
    try:
        cursor = conn.cursor()
        ...
        return render_template('imprimir_tablas.html', 
                               pacientes=pacientes, 
                               medicos=medicos, 
                               camas=camas)
    except pyodbc.Error as e:
